...
export default function OrderChatWindow({
  ...
  // Автоматична прокрутка повідомлень донизу
  useEffect(() => {
    if (!open) return;
  ...
  return (
      <section
          // Вікно жорстко закріплене в правому нижньому куті (fixed bottom-6 right-6)
          className="fixed bottom-0 right-0 z-[100] flex max-h-[85vh] w-full flex-col overflow-hidden border-t border-slate-200 bg-white/95 shadow-[0_-10px_40px_rgba(0,0,0,0.15)] backdrop-blur-xl transition-all sm:bottom-6 sm:right-6 sm:w-[400px] sm:rounded-[2rem] sm:border dark:border-white/10 dark:bg-slate-950/95"
          role="dialog"
          aria-modal="false" // Змінено на false, оскільки чат більше не блокує сторінку
          aria-label={title}
      >
        <header className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-white/10">
          ...
          <button
              type="button"
              onClick={onClose}
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-slate-200 text-slate-600 transition hover:bg-slate-100 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/5"
        ...
        <div className="border-b border-slate-200 px-5 py-3 text-xs uppercase tracking-[0.2em] text-slate-400 dark:border-white/10">
          {senderLabel} ↔ {staffLabel}
        </div>

        <div ref={listRef} className="flex-1 space-y-3 overflow-y-auto px-5 py-4 scroll-smooth">
          {loading ? (
        ...
        />
          <div className="mt-3 flex items-center justify-between gap-3">
            <p className="text-xs text-slate-500 dark:text-slate-400">Ctrl/⌘ + Enter — відправка</p>
            <button
                type="button"
                onClick={onSend}
                disabled={sending}
                className="rounded-2xl bg-slate-950 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-amber-400 dark:text-slate-950 dark:hover:bg-amber-300"
            >
