          ...
          {users.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-4 py-4">
                <select value={item.role} onChange={async (e) => { await axios.put(`http://localhost:8000/api/users/${item.id}`, { role: e.target.value }, auth); fetchUsers(); }} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-950/60">
              ...
              <td className="px-4 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <select value={item.is_active ? 'active' : 'inactive'} onChange={async (e) => { await axios.put(`http://localhost:8000/api/users/${item.id}`, { is_active: e.target.value === 'active' }, auth); fetchUsers(); }} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-950/60">
