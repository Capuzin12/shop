...
import { useAssignCustomerGroup, useCustomerGroups } from '../../../shared/hooks/useCustomerGroups';
import { isValidEmail, isValidPhone, normalizePhoneInput } from '../../../shared/utils/validation';
...
export default function AdminUsers() {
  ...
  const [formData, setFormData] = useState({ email: '', first_name: '', last_name: '', phone: '', role: 'customer', is_active: true });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setEditingUser(null);
      setFormData({ email: '', first_name: '', last_name: '', phone: '', role: 'customer', is_active: true });
      setFieldErrors({});
    } catch (error) {
  ...
  return (
      <div className="space-y-6">
        <Panel title="Користувачі" subtitle="Ролі, активність та контакти">
          ...
          <form noValidate onSubmit={submit} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 flex flex-wrap gap-3">
              ...
              {editingUser ? <button onClick={() => { setEditingUser(null); setFormData({ email: '', first_name: '', last_name: '', phone: '', role: 'customer', is_active: true }); setFieldErrors({}); setFormError(''); }} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 dark:border-white/10 dark:text-slate-200" type="button">Скасувати</button> : null}
            </div>
          </form>
        </Panel>
