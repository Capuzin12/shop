...
def fuzzy_product_score(search_query: str, product: Product) -> float:
    ...
    parts = [
        ...
        normalize_search_text(str(getattr(getattr(product, "brand", None), "name", ""))),
        normalize_search_text(str(getattr(getattr(product, "category", None), "name", ""))),
    ]
...
def search_products_response(
    ...
    *,
    ...
    if active_only:
        query = query.where(cast(Any, Product.is_active) == True)
        ...
        for term in search_terms:
            if ":" in term:
                ...
                if field == "brand" and value:
                    search_conditions.append(and_(cast(Any, Product.brand_id).isnot(None), Brand.name.ilike(f"%{value}%")))
                elif field == "category" and value:
                    search_conditions.append(and_(cast(Any, Product.category_id).isnot(None), Category.name.ilike(f"%{value}%")))
                    ...
                    search_conditions.append(or_(Product.name.ilike(f"%{term}%"), Product.description.ilike(f"%{term}%"), Product.sku.ilike(f"%{term}%"), and_(cast(Any, Product.brand_id).isnot(None), Brand.name.ilike(f"%{term}%")), and_(cast(Any, Product.category_id).isnot(None), Category.name.ilike(f"%{term}%"))))
            else:
                search_conditions.append(or_(Product.name.ilike(f"%{term}%"), Product.description.ilike(f"%{term}%"), Product.sku.ilike(f"%{term}%"), and_(cast(Any, Product.brand_id).isnot(None), Brand.name.ilike(f"%{term}%")), and_(cast(Any, Product.category_id).isnot(None), Category.name.ilike(f"%{term}%"))))
        if search_conditions:
            query = query.where(and_(*search_conditions))
    query = query.outerjoin(Brand, cast(Any, Product.brand_id) == Brand.id).outerjoin(Category, cast(Any, Product.category_id) == Category.id)
    if category_id:
        query = query.where(cast(Any, Product.category_id) == category_id)
    try:
        parsed_brand_ids = [int(value) for value in brand_ids.split(",") if value.strip()] if brand_ids else []
    except ValueError:
        parsed_brand_ids = []
    if parsed_brand_ids:
        query = query.where(cast(Any, Product.brand_id).in_(parsed_brand_ids))
    ...
    if sort_by == "price":
        ...
        discount_rank = select(func.max(ProductDiscount.discount_value)).where(ProductDiscount.product_id == Product.id, ProductDiscount.is_active == True, or_(ProductDiscount.start_date.is_(None), ProductDiscount.start_date <= now), or_(ProductDiscount.end_date.is_(None), ProductDiscount.end_date >= now)).scalar_subquery()
    ...
    if search and total == 0:
        fuzzy_query = select(Product).where(cast(Any, Product.is_active) == True).outerjoin(Brand, cast(Any, Product.brand_id) == Brand.id).outerjoin(Category, cast(Any, Product.category_id) == Category.id)
        if category_id:
            fuzzy_query = fuzzy_query.where(cast(Any, Product.category_id) == category_id)
        if parsed_brand_ids:
            fuzzy_query = fuzzy_query.where(cast(Any, Product.brand_id).in_(parsed_brand_ids))
        if min_price is not None:
            fuzzy_query = fuzzy_query.where(Product.price >= min_price)
        if max_price is not None:
            fuzzy_query = fuzzy_query.where(Product.price <= max_price)
        fuzzy_ranked = cast(list[tuple[Product, float]], list(rank_fuzzy_products(search, db.scalars(fuzzy_query.limit(500)).all())))
    ...
    brand_facets_query = brand_facets_query.outerjoin(Category, cast(Any, Product.category_id) == Category.id)
    if category_id:
        brand_facets_query = brand_facets_query.where(cast(Any, Product.category_id) == category_id)
    if min_price is not None:
    ...
    category_facets_query = select(Category.id, Category.name, func.count(Product.id)).join(Product, cast(Any, Product.category_id) == Category.id).where(cast(Any, Product.is_active) == True)
    if search and facet_search_conditions:
        category_facets_query = category_facets_query.where(and_(*facet_search_conditions))
    category_facets_query = category_facets_query.outerjoin(Brand, cast(Any, Product.brand_id) == Brand.id)
    if parsed_brand_ids:
        category_facets_query = category_facets_query.where(cast(Any, Product.brand_id).in_(parsed_brand_ids))
    if min_price is not None:
    ...
    price_stats_query = select(func.min(Product.price), func.max(Product.price), func.avg(Product.price)).where(cast(Any, Product.is_active) == True)
    if search and facet_search_conditions:
        price_stats_query = price_stats_query.where(and_(*facet_search_conditions))
    if category_id:
        price_stats_query = price_stats_query.where(cast(Any, Product.category_id) == category_id)
    if parsed_brand_ids:
        price_stats_query = price_stats_query.where(cast(Any, Product.brand_id).in_(parsed_brand_ids))
    ...
    availability_query = select(func.sum(case((Inventory.quantity > 0, 1), else_=0)), func.sum(case((Inventory.quantity == 0, 1), else_=0)), func.count(Product.id)).outerjoin(Inventory, Product.id == Inventory.product_id).where(cast(Any, Product.is_active) == True)
    if search and facet_search_conditions:
        availability_query = availability_query.where(and_(*facet_search_conditions))
    if category_id:
        availability_query = availability_query.where(cast(Any, Product.category_id) == category_id)
    if parsed_brand_ids:
        availability_query = availability_query.where(cast(Any, Product.brand_id).in_(parsed_brand_ids))
    if min_price is not None:
